...
from summarizer.gupta import config
from summarizer.gupta.trainer.trainer import Trainer, hsizes
from summarizer.gupta.data.dataloader import AmazonDataLoader
from summarizer.gupta.data.dataset import AmazonDataset
from summarizer.gupta.models.seq2seq import Seq2Seq
from summarizer.gupta.utils.logger import Logger
from summarizer.gupta.utils.saver import Saver
...
def main():
    ...
    if mode == C.TRAIN_TYPE:
        logger.log('\nLoading dataset..')
        dataset = AmazonDataset(args, params, mode)
        logger.log('\n Model: %s, Mode = %s \n' % (model_name, mode))
        logger.log('\nLoading dataloader..')
