...
from summarizer.gupta.data.vocabulary import Vocabulary
from summarizer.gupta.data import review_utils
import string
import json
...
class AmazonDataset(object):
    def __init__(self, args, params, mode):
    ...
    def create_vocab(self, train_path):
        ...
        df = pd.read_json(train_path, orient='split')
        df = df.head(500)
        
        questions = [q if q.endswith("?") else q+"?" for q in df.question]
        customer_reviews = [r for r in df.passages]
        customer_answers = [a for a in df.answers]
        for i in range(len(df)):
            tokens = self.truncate_tokens(questions[i], self.max_question_len)
            vocab.add_sequence(tokens)

            tokens = self.truncate_tokens(customer_answers[i], self.max_answer_len)
            total_tokens += len(tokens)
            vocab.add_sequence(tokens)

            for review in customer_reviews[i]:
    ...
    def get_data(self, path):
        ...
        for i in range(len(questions)):
            ...
            answers = [customer_answers[i]]
            reviews = customer_reviews[i][:5]    # TODO: Make this limit (5) a parameter
            ...
            tuples = []
            for answer in answers:
                answer_tokens = self.truncate_tokens(answer, self.max_answer_len)
                answer_ids = self.vocab.indices_from_token_list(answer_tokens)
                answersDict.append(answer_ids)
                answerId += 1
                answerIdsList.append(answerId)
